...
export class GameScene extends Phaser.Scene {
     ...
     */
    initMultiplayerGame() {
        const data = this.initialGameData || { players: [] };

        // Create players from server data (online connection preserved)
        if (data.players && data.players.length > 0) {
            data.players.forEach(pData => {
                const isLocal = pData.id === networkManager.getLocalPlayerId();
                const player = new Player(this, pData.x, pData.y, {
                    id: pData.id,
                    name: pData.name,
                    color: pData.color,
                    size: pData.size,
                ...
                player.dimension = pData.dimension || null; // Support dimensions
        ...
        // Fallback local player if server data is missing
        if (!this.localPlayer) {
            this.localPlayer = new Player(this, this.worldWidth / 2, this.worldHeight / 2, {
                id: networkManager.getLocalPlayerId() || 'local',
                name: this.playerName,
                color: getRandomPlayerColor(),
                isLocal: true
            });
        ...
        // Create bots locally (solo parity)
        this.bots = createBots(this, CONFIG.BOT_COUNT);

        // Create orbs locally (solo parity)
        this.orbs = createOrbs(this, CONFIG.INITIAL_ORB_COUNT);

        // Set entity references for bots (AI threat/prey detection)
        this.bots.forEach(bot => {
            bot.setOrbs(this.orbs);
            bot.setPlayers(this.players);
            bot.setOtherBots(this.bots);
    ...
    setupNetworkHandlers() {
        networkManager.on('gameState', (data) => this.handleGameState(data));
        networkManager.on('playerMoved', (data) => this.handlePlayerMoved(data));
        networkManager.on('orbCollected', (data) => this.handleOrbCollected(data));
        networkManager.on('playerAbsorbed', (data) => this.handlePlayerAbsorbed(data));
        networkManager.on('playerRespawned', (data) => this.handlePlayerRespawned(data));
        networkManager.on('gameEnded', (data) => this.handleGameEnded(data));
        networkManager.on('dimensionEntered', (data) => this.handleDimensionEntered(data));
        networkManager.on('dimensionExited', (data) => this.handleDimensionExited(data));
        networkManager.on('orbSync', (data) => this.handleOrbSync(data));
        networkManager.on('disconnect', () => this.handleDisconnect());
    }
     ...
     */
    updateMultiplayerGame(time, delta) {
        if (!this.localPlayer) return;
        ...
        // Get current world size and speed multiplier (dimension-aware)
        const worldSize = this.getCurrentWorldSize();
        const speedMult = this.getDimensionSpeedMultiplier();

        // Get keyboard direction and move local player
        const dir = this.getKeyboardDirection();
        if (dir.x !== 0 || dir.y !== 0) {
            this.localPlayer.moveInDirection(dir.x, dir.y, speedMult);
            this.localPlayer.clampToWorld(worldSize.width, worldSize.height);
        ...
        // Send position update to server (throttled to 25ms = 40Hz)
        const now = Date.now();
        if (now - this.lastMoveUpdate > 25) {
            networkManager.sendPlayerMove(this.localPlayer.x, this.localPlayer.y);
            this.lastMoveUpdate = now;
        }

        // Dimension-specific updates (solo parity)
        if (this.currentDimension) {
            this.updateDimensionBots(delta);
            this.checkDimensionCollisions();
            this.checkMazeWallCollisions();
            this.applyDimensionDecay();
        } else {
            this.checkRiftEntry();
            this.updateBots(delta);
        ...
        // Interpolate other players (online connection)
        this.players.forEach(player => {
            if (!player.isLocal) {
                player.interpolatePosition();

                // Update visibility based on dimension
                if (this.localPlayer.dimension !== player.dimension) {
                    player.setVisible(false);
                    player.nameLabel.setVisible(false);
                } else {
                    player.setVisible(player.alive);
                    player.nameLabel.setVisible(player.alive);
                }
            }
        });
    }
    ...
    handleGameState(data) {
        // Multiplayer solo-simulation: only sync remote players from server
        if (this.gameMode === 'multiplayer' && this.multiplayerUsesSoloSimulation) {
            data.players.forEach(pData => {
                const player = this.players.find(p => p.id === pData.id);
        ...
        data.players.forEach(pData => {
            let player = this.players.find(p => p.id === pData.id);
            if (player) {
                player.deserialize(pData);
            }
        });

        // Update bots from server state
        if (data.bots) {
            data.bots.forEach(bData => {
                let bot = this.bots.find(b => b.id === bData.id);
                if (bot) {
                    bot.targetX = bData.x;
                    bot.targetY = bData.y;
                    bot.setSize(bData.size);
                    bot.alive = bData.alive !== false;
                    bot.dimension = bData.dimension; // Sync dimension state
                    bot.setVisible(bot.dimension === this.localPlayer.dimension && bot.alive);
                }
            });
        }
    }
    ...
    handleOrbCollected(data) {
        if (this.gameMode === 'multiplayer' && this.multiplayerUsesSoloSimulation) return;
        // Get the collected orb before removing it for visual effects
        const collectedOrb = this.orbs.find(o => o.id === data.orbId);
        const orbColor = collectedOrb?.orbColor;
        const orbSize = collectedOrb?.orbSize;
        ...
        if (orbIndex !== -1) {
            this.orbs[orbIndex].collect();
            this.orbs.splice(orbIndex, 1);
        }

        // Add new orb
        if (data.newOrb) {
            const newOrb = new Orb(this, data.newOrb.x, data.newOrb.y, {
                id: data.newOrb.id,
                color: data.newOrb.color,
                size: data.newOrb.size
            });
            newOrb.dimension = data.newOrb.dimension;
            this.orbs.push(newOrb);
        }

        // Update player or bot that collected the orb
        let player = this.players.find(p => p.id === data.playerId);
        if (!player) {
            player = this.bots.find(b => b.id === data.playerId);
        }
        if (player) {
            player.setSize(data.newSize);
            player.score = data.newScore;

            if (player.isLocal) {
                // Update combo (same as solo mode)
                this.incrementCombo();
                ...
                // Show absorbed orb effect
                if (player.showAbsorbedOrb && orbColor) {
                    player.showAbsorbedOrb(orbColor, orbSize);
                }

                // Create ripple effect at collection point (scaled by combo)
                if (orbX !== undefined && orbY !== undefined) {
                    this.createRippleEffect(orbX, orbY, player.playerSize, this.combo.count);
                }

                this.stats.score = data.newScore;
                this.stats.orbsCollected++;
            }
        }
    }

    handlePlayerAbsorbed(data) {
        if (this.gameMode === 'multiplayer' && this.multiplayerUsesSoloSimulation) return;
        // Search both players and bots for absorbed entity
        let absorbed = this.players.find(p => p.id === data.absorbedId);
        if (!absorbed) {
            absorbed = this.bots.find(b => b.id === data.absorbedId);
        }
        // Search both players and bots for absorber entity
        let absorber = this.players.find(p => p.id === data.absorberId);
        if (!absorber) {
            absorber = this.bots.find(b => b.id === data.absorberId);
        }
        ...
        if (absorber) {
            ...
            if (absorber.isLocal) {
                ...
                this.stats.playersAbsorbed++;
                soundManager.playAbsorb();
                // Match solo mode: screen shake when killing
                this.triggerScreenShake(5, 10);
            }
        }

        if (absorbed && absorbed.isLocal) {
            soundManager.playDeath();
            this.triggerScreenShake(12, 25);
            this.showNotification('💀 ABSORBED!', '#ef4444');
        }
    ...
    handleDimensionEntered(data) {
        if (this.gameMode === 'multiplayer' && this.multiplayerUsesSoloSimulation) return;
        // Find the player who entered the dimension (could be local or remote)
        let player = this.players.find(p => p.id === data.playerId);

        if (player) {
            // Update player's dimension state
            player.dimension = data.dimensionType;
            player.setPosition(data.x, data.y);
            player.setInvulnerable(true);

            // If this is the local player, enter dimension mode
            if (player.isLocal) {
                this.enterDimension(data.dimensionType, null);
                this.dimensionTimer = data.duration;
            }
        }
    }

    handleDimensionExited(data) {
        if (this.gameMode === 'multiplayer' && this.multiplayerUsesSoloSimulation) return;
        // Find the player who exited the dimension
        let player = this.players.find(p => p.id === data.playerId);
        ...
        if (player) {
            ...
            player.setPosition(data.x, data.y);
            // No invulnerability on exit - matches server and solo mode
            player.setInvulnerable(false);

            // If this is the local player, exit dimension mode
            if (player.isLocal) {
                this.exitDimension();
            }
        }
    ...
    /**
     * Handle orb synchronization to fix ghost orbs
     */
    handleOrbSync(data) {
        if (this.gameMode === 'multiplayer' && this.multiplayerUsesSoloSimulation) return;
        // Create a Set of server orb IDs for fast lookup
        const serverOrbIds = new Set(data.orbs.map(orb => orb.id));

        // 1. Remove local orbs that are not on the server (Ghosts)
        for (let i = this.orbs.length - 1; i >= 0; i--) {
            const orb = this.orbs[i];
            if (!serverOrbIds.has(orb.id)) {
                orb.destroy();
                this.orbs.splice(i, 1);
            }
        }

        // 2. Add missing orbs from server
        data.orbs.forEach(serverOrb => {
            // Find if we already have this orb
            const existingOrb = this.orbs.find(o => o.id === serverOrb.id);

            if (!existingOrb) {
                // Determine if we should spawn it based on dimension
                // Only spawn if it matches our current dimension (or null for main world)
                const inSameDimension = (this.localPlayer.dimension === serverOrb.dimension);

                if (inSameDimension) {
                    const newOrb = new Orb(this, serverOrb.x, serverOrb.y, serverOrb.size, serverOrb.color, serverOrb.id);
                    newOrb.dimension = serverOrb.dimension;
                    this.orbs.push(newOrb);
                }
            } else {
                // Reconciliation: Update position/dimension just in case
                // (Orbs are mostly static but this adds robustness)
                existingOrb.dimension = serverOrb.dimension;
            }
        });
    }
    ...
    handleDisconnect() {
        ...
        this.gameRunning = false;
        // Use alert for immediate feedback or just switch scenes
        // Given user dislike for interruptions, maybe just switch scene with a toast?
        // But this is a critical failure. Alert is safer to explain WHY they are at menu.
